      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[1px] bg-white/5 border border-white/5">
          {ServicesData.map((service, index) => (
            // 1. Link hamesha bahar hona chahiye aur 'key' isi par aayegi
            <Link key={index} href={`/services/${index + 1}`} className="block group relative overflow-hidden">
              <motion.div
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                viewport={{ once: true }}
                transition={{ duration: 0.8, delay: index * 0.1 }}
                className="bg-[#050505] p-12 md:p-20 flex flex-col justify-between aspect-square md:aspect-video lg:aspect-square cursor-crosshair"
              >
